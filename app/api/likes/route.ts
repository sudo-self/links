...
type LikesOverviewResponse = {
  success: boolean;
  pages?: { page_id: string; like_count: number; updated_at: string }[];
  total?: number;
  error?: string;
  details?: string;
};
...
export async function GET(): Promise<NextResponse<LikesOverviewResponse>> {
  try {
...
export async function POST(request: NextRequest): Promise<NextResponse<{ success: boolean; likes?: number; error?: string }>> {
